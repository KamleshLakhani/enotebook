...
const JWT_SECRET='webdevelopment'

//ROUTE 1 create a user using: POST "/api/auth/". No login requireds
router.post('/createuser',[
   body('email','Enter a valid email').isEmail(),
   body('name','Enter a valid name').isLength({min:3}),
   body('password','Passord must be atleast 5 characters').isLength({min:5})
   ], async(req,res)=>{
    let success=false;
    ...
    //Check wether the user with the email already exist or not
    try{
